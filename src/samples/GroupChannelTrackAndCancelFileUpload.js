...
const GroupChannelTrackAndCancelFileUpload = (props) => {
    ...
    const onFileCancel = () => {
        const { currentlyJoinedChannel, reqId, fileLoading } = state;

        currentlyJoinedChannel.cancelUploadingFileMessage(reqId)
          .then((canceled) => {
            canceled && updateState({ ...state, fileLoading: !fileLoading });
        })
          .catch((error) => {
    ...
    const onFileInputChange = async (e) => {
        if (e.currentTarget.files && e.currentTarget.files.length > 0) {
            ...
            updateState({ ...state, fileLoading: true, progress: 10 });

            currentlyJoinedChannel.sendFileMessage(fileMessageParams)
                .onPending((message) => {
                    updateState({ ...state, fileLoading: true, reqId: message.reqId, progress: 90 });
                })
                .onSucceeded((message) => {
                    const updatedMessages = [...messages, message];
                    updateState({ ...state, messages: updatedMessages, messageInputValue: "", file: null, fileLoading: false, progress: 100 });
                })
                .onFailed((error) => {
                    console.log(error)
                    console.log("failed")
...
const MessageInput = ({ value, onChange, sendMessage, onFileInputChange, fileLoading, onFileCancel, progress }) => {
    return (
      <div className="message-input">
          {
              fileLoading && progress < 100 && (
                <div className="progress">
                    <span className={fileLoading ? "animated" : ""} style={{ width: `${progress}%` }} />
                </div>
              )
          }
          <input
...
export default GroupChannelTrackAndCancelFileUpload;