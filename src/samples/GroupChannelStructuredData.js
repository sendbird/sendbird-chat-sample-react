        ...
        if (messageToUpdate) {
            ...
            currentlyJoinedChannel.sendUserMessage(userMessageParams)
                .onSucceeded((message) => {
                    ...
                    updateState({ ...state, messages: updatedMessages, messageInputValue: "", messageData: {} });
                })
                .onFailed((error) => {
                    console.log(error)
                    console.log("failed")
...
const Message = ({ message, updateMessage, handleDeleteMessage, messageSentByYou, data }) => {
    if (message.url) {
        return (
    ...
    return (
        <div className={`message message--group ${messageSentByYou ? 'message-from-you' : ''}`}>
...
const MessageInput = ({ value, onChange, sendMessage, onFileInputChange, toggleShowAddDataToMessage, handleKeyDown }) => {
    return (
