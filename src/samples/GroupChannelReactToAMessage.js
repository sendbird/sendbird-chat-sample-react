...
const GroupChannelReactToAMessage = (props) => {
    ...
    const addMessageReaction = async (message, e) => {
        ...
        const reactionEvent = await currentlyJoinedChannel.addReaction(
            message,
            emojiKey
        )
    ...
    const removeMessageReaction = async (message, messageKey) => {
        const { currentlyJoinedChannel } = state
        const reactionEvent = await currentlyJoinedChannel.deleteReaction(
            message,
            messageKey
        )
        message.applyReactionEvent(reactionEvent)
...
const Message = ({
    ...
    const messageSentByCurrentUser =
        message.sender.userId === sb.currentUser.userId
    const showReactions =
        isReactions && currentMessage.messageId === message.messageId
    ...
    return (
        ...
        >
                    ...
                    {message.reactions.map((react, i) => {
                        return (
                            <span
                                ...
                                key={i + react.key}
                                onClick={() =>
                                    removeMessageReaction(message, react.key)
                                }
                            >
                                {react.key}
                                <sup className="reactions-item-inner">
                                    {react.userIds.length > 1
            ...
            <div className="react-button-wrapper">
                {showReactions && (
                    <ul className="reactions-list">
                        <li>
                            <button
                                className="control-button"
                                onClick={(e) => addMessageReaction(message, e)}
                            >
                                &#128512;
                            </button>
                        </li>
                ...
                <button
                    className="control-button react-button"
                    onClick={() => toggleReactions(message)}
                >
                    <span className="message-icon react-button-img">
                        &#128512;
                    </span>
                </button>
            </div>
