...
const GroupChannelMembersListOrder = (props) => {
    ...
    const handleSortMembers = async (event) => {
        if (event.target.value !== 'all') {
            const [updatedMembers, error] = await loadMembers(
                state.currentlyJoinedChannel.url,
                event.target.value
            )
            if (error) {
                return onError(error)
    ...
    if (members) {
        return (
            <div className="members-list">
                ...
                {(sortMembers !== 'all' ? members : channel?.members)?.map(
                    (member) => {
                        const isOperator = member.role === 'operator'
                        const memberIsSender =
                            member.userId !== userIdInputValue
                        return (
