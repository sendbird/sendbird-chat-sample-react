...
import { handleEnterPress, timestampToTime } from '../utils/messageUtils'
let sb

const GroupChannelRetrieveNumberOfMembersHaventReadMessage = (props) => {
    ...
    const getNumberOfUnreadMembers = (message) => {
        const { currentlyJoinedChannel } = state
        const numberOfUnreadMembers =
            currentlyJoinedChannel.getUnreadMemberCount(message)

        updateState({
            ...state,
            numberOfUnreadMembers,
...
const MessagesList = ({
    ...
    updateMessage,
    numberOfUnreadMembers,
    getNumberOfUnreadMembers,
    currentMessage,
    closeNumberOfUnreadMembers,
}) => {
    return (
...
const Message = ({
    ...
    const messageSentByCurrentUser =
        message.sender.userId === sb.currentUser.userId
    const showNumberOfUnreadMembers =
        numberOfUnreadMembers && currentMessage.messageId === message.messageId
            ...
            >
                <div className="message-info">
                    ...
                    {messageSentByCurrentUser && (
                        <div>
                            <button
                                className="control-button number-of-undelivered-message-btn"
                                data-title="Get number of unread members"
                                onClick={() =>
                                    getNumberOfUnreadMembers(message)
                                }
                            >
                                <img
                ...
                <img src={message.url} />
                {showNumberOfUnreadMembers && (
                    <div className="number-of-undelivered-members">
                        Number of members unread a message:{' '}
                        {numberOfUnreadMembers}
                        <span
                            className="number-of-undelivered-members-btn"
                            onClick={closeNumberOfUnreadMembers}
                        >
                            &#10006;
                        </span>
                    </div>
    ...
    return (
        ...
        >
                ...
                {messageSentByCurrentUser && (
                    <div>
                        ...
                        <button
                            className="control-button number-of-undelivered-message-btn"
                            data-title="Get number of unread members"
                            onClick={() => getNumberOfUnreadMembers(message)}
                        >
                            <img
                                className="message-icon"
                                src="/unread_icon.png"
            ...
            {showNumberOfUnreadMembers && (
                <div className="number-of-undelivered-members">
                    Number of members unread a message: {numberOfUnreadMembers}
                    <span
                        className="number-of-undelivered-members-btn"
                        onClick={closeNumberOfUnreadMembers}
                    >
