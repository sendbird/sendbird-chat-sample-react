...
const BasicGroupChannelSample = (props) => {
    ...
    const messageHandlers = {
        ...
        onMessagesDeleted: (context, channel, messageIds) => {
            const updateMessages = stateRef.current.messages.filter(
                (message) => {
    ...
    const sendScheduledMessage = async () => {
        ...
        if (
            state.scheduleTime.replace(':', '') >
            getDate().min.slice(-5).replace(':', '')
        ) {
            if (scheduleMessageToUpdate) {
                ...
                userMessageParams.message = state.messageInputValue
                currentlyJoinedChannel
                    .createScheduledUserMessage(userMessageParams)
                    .onPending((message) => {
            ...
            updateState({
                ...stateRef.current,
                scheduleMessageRangeError: true,
            })
        }
    }
    ...
    const updateScheduleMessage = async (message) => {
        updateState({
            ...state,
            scheduleMessageToUpdate: message,
            scheduleMessageInputValue: message.message,
        })
    }
    ...
    const formatDate = (unixTimestamp) => {
        ...
        return `${year}-${month.slice(-2)}-${day.slice(-2)}T${hours.slice(
            -2
        )}:${minutes.slice(-2)}`
    }
    ...
    const toggleShowScheduledMessageSettingsModal = () => {
        updateState({
            ...
            showScheduledMessageSettingsModal:
                !state.showScheduledMessageSettingsModal,
        })
    }
    ...
    const handleDeleteScheduleMessage = async (scheduledMessage) => {
        const { currentlyJoinedChannel } = state
        await currentlyJoinedChannel.cancelScheduledMessage(
    ...
    const sendScheduleMessageImmediately = async (scheduledMessage) => {
        const { currentlyJoinedChannel, messages } = state
        await currentlyJoinedChannel.sendScheduledMessageNow(
            scheduledMessage.scheduledInfo.scheduledMessageId
        )
    ...
    const rescheduleMessage = (message) => {
        updateState({
            ...state,
            scheduleMessageToUpdate: message,
            showScheduledMessageSettingsModal: true,
        })
    }
        ...
        <>
            ...
            <ScheduledMessageListModal
                showScheduledMessageListModal={
                    state.showScheduledMessageListModal
                }
                toggleShowScheduledMessageListModal={
                    toggleShowScheduledMessageListModal
                }
...
const ChannelName = ({ members }) => {
    ...
    return (
        <>
            ...
            {membersNotToDisplay.length > 0 &&
                `+ ${membersNotToDisplay.length}`}
        </>
    )
}
        ...
        return (
            ...
            >
                <div className="message-user-info">
                    ...
                    <div>
                        {timestampToTime(message.scheduledInfo.scheduledAt)}
                    </div>
                </div>
                <img src={message.url} />
            </div>
                ...
                {messageSentByCurrentUser && (
                    <div>
                        <button
                            ...
                            onClick={() =>
                                sendScheduleMessageImmediately(message)
                            }
                        >
                            <img
                                className="message-icon"
                                src="/icon_send-message.png"
                                alt="Send message"
                            />
                        </button>
                        <button
                            ...
                            onClick={() => rescheduleMessage(message)}
                        >
                            <img
                                className="message-icon"
                                src="/icon_reschedule.png"
                                alt="Reschedule message"
                            />
                        </button>
                        <button
                            className="control-button"
                            data-tooltip="edit"
                            onClick={() => updateScheduleMessage(message)}
                        >
                            <img
                                className="message-icon"
                                src="/icon_edit.png"
                        ...
                        <button
                            className="control-button"
                            data-tooltip="delete"
                            onClick={() => handleDeleteScheduleMessage(message)}
                        >
                            <img
                                className="message-icon"
                                src="/icon_delete.png"
                    ...
                    {applicationUsers.map((user) => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() =>
                                    addToChannelMembersList(user.userId)
                                }
                            >
...
const ScheduledMessageSettingsModal = ({
    ...
    if (showScheduledMessageSettingsModal) {
        return (
            <div className="overlay scheduled-messages-settings-modal">
                <div
                    className="overlay-content"
                    onKeyDown={(event) => handleEnterPress(event, setupUser)}
                >
                    <div className="settings-header">
                        {scheduleMessageToUpdate
                            ? 'Reschedule message'
                            : 'Send shcheduled message'}
                    </div>
                    <div className="schedule-message-error">
                        {scheduleMessageRangeError &&
                            'Scheduled time should be between 5 minutes and 30 days'}
                    </div>
                    <input
                        type="datetime-local"
                        onChange={(e) => {
...
const ScheduledMessageListModal = ({
    ...
    if (showScheduledMessageListModal) {
        return (
            <div className="overlay ">
                <div
                    className="overlay-content scheduled-messages-list-modal"
                    onKeyDown={(event) => handleEnterPress(event, setupUser)}
                >
