...
const OpenChannelMembersListOrder = (props) => {
  ...
  const handleSortMembers = async (event) => {
    const { applicationUsers, currentlyJoinedChannelOperators } = state;
    switch(event.target.value) {
      case "member_nickname_alphabetical":
        const updatedUsers = sortUsersAlphabetical(applicationUsers);
        updateState({ ...state, applicationUsers: updatedUsers });
        break;
      case "operator_then_member_alphabetical":
        const alphabeticalUsers = sortUsersAlphabetical(applicationUsers);
        const updatedUsersList = alphabeticalUsers.sort((user) => {
          const isOperator = currentlyJoinedChannelOperators.find((operator) => user.userId === operator.userId.substr(0, 2));
          if(isOperator) {
            return -1;
          }
          return 0;
        })
        updateState({ ...state, applicationUsers: updatedUsersList });
        break;
      case "all":
        const [users, usersError] = await getAllApplicationUsers();
...
const MembersList = ({ toggleMembersList, isShowMembersList, users, registerUnregisterAnOperator, userIdInputValue, operators, handleSortMembers }) => {
  if (isShowMembersList && users) {
      return <div className="members-list">
          ...
          {users.map((user) => {
              const isOperator = operators.find((operator) => user.userId === operator.userId.substr(0, 2));
...
export default OpenChannelMembersListOrder;