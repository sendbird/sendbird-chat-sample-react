...
const GroupChannelMessageThreading = (props) => {
    ...
    const sendThreadMessage = async () => {
        ...
        const userMessageParams = {
            parentMessageId: threadParentsMessage.messageId,
        }
        userMessageParams.message = state.threadMessageInputValue
        userMessagesHandler(true, userMessageParams, threadMessages)
    }

    const fileMessagesHandler = (
        fileMessageParams,
        messages,
        isThread,
        event
    ...
    const openThread = async (parentsMessage) => {
        ...
        const messageSentByYou =
            parentsMessage.sender.userId === sb.currentUser.userId
...
const MessageInput = ({
    ...
    isOpenThread,
    threadInputClass = '',
    onFileThreadInputChange,
    isThread = false,
}) => {
    return (
        <div
            className={`message-input ${threadInputClass} ${
                isOpenThread ? 'message-input-column' : ''
            }`}
        >
            <input
