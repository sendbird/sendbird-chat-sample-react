...
const OpenChannelBanUnbanUsers = (props) => {
    ...
    const banUnbanUser = async (user, index, callbackName) => {
        ...
        const isOperator = currentlyJoinedChannelOperators.find(
            (operator) => userIdInputValue === operator.userId
        )
        ...
        if (isOperator) {
            try {
                if (callbackName === 'ban') {
                    await currentlyJoinedChannel.banUser(
                        user,
                        1000000,
                        'just banned'
                    )
...
const MembersList = ({
    ...
    if (isShowMembersList && users) {
        return (
            <div className="members-list oc-members-list">
