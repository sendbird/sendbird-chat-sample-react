...
const OpenChannelMetadataAndMetacounter = (props) => {
    ...
    const handleUpdateChannel = async () => {
        const { currentlyUpdatingChannel, channelNameInputValue, channels } =
            state
    ...
    const sendMessage = async () => {
        ...
        if (messageToUpdate) {
            ...
            const updatedMessage =
                await currentlyJoinedChannel.updateUserMessage(
    ...
    const setupUser = async () => {
        ...
        await sendbirdChat.connect(
            userIdInputValue,
            '79d98dc7e160e104ded1eb6b86de46bb6ae27630'
        )
    ...
    const toggleMetadataModal = async () => {
        ...
        updateState({
            ...state,
            isOpenMetadataModal: !isOpenMetadataModal,
            currentMetadataObject: { ...data },
        })
    }
    ...
    const addMetadataObjectItem = async () => {
        ...
        if (metadataItemToUpdate) {
            ...
            if (metadataItemToUpdate !== metadataKeyInputValue) {
                await currentlyJoinedChannel.deleteMetaData(
                    metadataItemToUpdate
                )

                await currentlyJoinedChannel.createMetaData({
                    [metadataKeyInputValue]: metadataValueInputValue,
                })
            } else {
                await currentlyJoinedChannel.updateMetaData(
                    update,
                    upsertIfKeyNotExist
                )
            }

            const { [metadataItemToUpdate]: remove, ...rest } =
                currentMetadataObject
    ...
    const updateMetadataObjectItem = (key, value) => {
        updateState({
            ...state,
            metadataItemToUpdate: key,
            metadataKeyInputValue: key,
            metadataValueInputValue: value,
        })
    }
    ...
    const toggleMetacounterModal = async () => {
        ...
        updateState({
            ...state,
            isOpenMetacounterModal: !isOpenMetacounterModal,
            currentMetacounterObject: { ...data },
        })
    }
    ...
    const addMetacounterObjectItem = async () => {
        ...
        if (metacounterItemToUpdate) {
            ...
            if (metacounterItemToUpdate !== metacounterKeyInputValue) {
                await currentlyJoinedChannel.deleteMetaCounters(
                    metacounterItemToUpdate
                )

                await currentlyJoinedChannel.createMetaCounters({
                    [metacounterKeyInputValue]: metacounterValueInputValue,
                })
            } else {
                await currentlyJoinedChannel.updateMetaCounters(
                    update,
                    upsertIfKeyNotExist
                )
            }

            const { [metacounterItemToUpdate]: remove, ...rest } =
                currentMetacounterObject
            ...
            updateState({
                ...
                metacounterObject: newObject,
                currentMetacounterObject: {
                    ...state.currentMetacounterObject,
                    ...newObject,
                },
                metacounterKeyInputValue: '',
                metacounterValueInputValue: '',
    ...
    const updateMetacounterObjectItem = (key, value) => {
        updateState({
            ...state,
            metacounterItemToUpdate: key,
            metacounterKeyInputValue: key,
            metacounterValueInputValue: value,
        })
    }
                        ...
                        {userIsOperator && (
                            <div>
                                ...
                                <button
                                    className="control-button"
                                    onClick={() =>
                                        handleDeleteChannel(channel.url)
                                    }
                                >
                                    <img
                                        className="channel-icon"
            ...
            <div className="channel">
                <div className="channel-header-wrapper">
                    ...
                    <div className="create-metadata-conteiner">
                        ...
                        <button
                            className="channel-create-button create-metadata-btn"
                    ...
                    <div className="create-metadata-conteiner">
                        ...
                        <button
                            className="channel-create-button create-metadata-btn"
                            onClick={() => toggleMetacounterModal()}
                        >
                            Edit
                        </button>
                    </div>
...
const MetadataModal = ({
    isOpenMetadataModal,
    toggleMetadataModal,
    addMetadataObjectItem,
    metadataKeyInputValue,
    onMetadataKeyInputValue,
    onMetadataValueInputValue,
    metadataValueInputValue,
    currentMetadataObject,
    updateMetadataObjectItem,
    handleDeleteMetadataObjectItem,
}) => {
                ...
                <div className="overlay-content">
                    ...
                    {!isEmpty &&
                        keys.map((key) => {
                            return (
                                <div
                                    key={`${key}`}
                                    className="metadata-modal-list"
                                >
                                    <div>
                                        {`${key}: `}
                                        {`${currentMetadataObject[key]}`}
                                    </div>
                                    <button
                                        className="control-button"
                                        onClick={() =>
                                    ...
                                    <button
                                        className="control-button"
                                        onClick={() =>
                                            handleDeleteMetadataObjectItem(key)
                                        }
                                    >
                                        <img
                                            className="oc-message-icon"
                    ...
                    <div className="metadata-modal-input">
                        <input
                            type="text"
                            placeholder="key"
                            onChange={(e) => onMetadataKeyInputValue(e)}
                            name="key"
                            value={metadataKeyInputValue}
                        ></input>
                        <input
                            type="text"
                            placeholder="value"
                            onChange={(e) => onMetadataValueInputValue(e)}
                            name="value"
                            value={metadataValueInputValue}
                        ></input>
                        <button onClick={() => addMetadataObjectItem()}>
                            Add
                        </button>
                    </div>
                    <button
...
const MetacounterModal = ({
    isOpenMetacounterModal,
    toggleMetacounterModal,
    addMetacounterObjectItem,
    metacounterKeyInputValue,
    onMetacounterKeyInputValue,
    onMetacounterValueInputValue,
    metacounterValueInputValue,
    currentMetacounterObject,
    updateMetacounterObjectItem,
    handleDeleteMetacounterObjectItem,
}) => {
                ...
                <div className="overlay-content">
                    ...
                    <div className="metadata-modal-input">
                        <input
                            type="text"
                            placeholder="key"
                            onChange={(e) => onMetacounterKeyInputValue(e)}
                            name="key"
                            value={metacounterKeyInputValue}
                        ></input>
                        <input
                            type="number"
                            placeholder="value"
                            onChange={(e) => onMetacounterValueInputValue(e)}
                            name="value"
                            value={metacounterValueInputValue}
                        ></input>
                        <button onClick={() => addMetacounterObjectItem()}>
                            Add
                        </button>
                    </div>
                    <button
