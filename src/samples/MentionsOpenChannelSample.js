...
const MentionOpenChannelSample = (props) => {
    ...
    const onMessageInputChange = (e) => {
        ...
        if (messageInputValue.includes('@')) {
            const userId = messageInputValue.match(/@[\d\/\w]*/g).map((value) => {
                return value.replace("@","");
            });

            updateState({ ...state, messageInputValue, userIdsList: userId });
        }
    }

    const sendMessage = async () => {
        const { messageToUpdate, currentlyJoinedChannel, messages, userIdsList } = state;
        ...
        if (messageToUpdate) {
            ...
            userMessageParams.mentionedUserIds = [...userIdsList];
            currentlyJoinedChannel.sendUserMessage(userMessageParams).onSucceeded((message) => {
                const updatedMessages = [...messages, message];
                updateState({ ...state, messages: updatedMessages, messageInputValue: "", userIdsList: [] });
...
export default MentionOpenChannelSample;